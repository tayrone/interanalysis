# The following code maps differentially methylated probes to its genes.

library(gdata)

gdata::keep(diff_methylated, sure = T)

dm_genes <- unique(diff_methylated$hgnc_symbol)

save(dm_genes, file = "./interanalysis_files/rdata_files/3_dm_genes.RData")

# #### The code bellow takes into account DM regions, but it returns almost all
# # existing genes as DM.
# 
# library(dplyr)
# library(biomaRt)
# library(stringr)
